...
GO
	IF EXISTS (SELECT name FROM sysobjects WHERE name='ES_COMPUESTO_POR_PRODUCTO')
		DROP FUNCTION ES_COMPUESTO_POR_PRODUCTO
	IF EXISTS (SELECT name FROM sysobjects WHERE name='TIENE_PRODUCTO_COMPONENTE')
		DROP FUNCTION TIENE_PRODUCTO_COMPONENTE
...
GO
	CREATE FUNCTION ES_COMPUESTO_POR_PRODUCTO (@producto char(8))
	RETURNS TINYINT
	AS
	BEGIN
...
GO
	CREATE FUNCTION TIENE_PRODUCTO_COMPONENTE (@factura char(8))
	RETURNS TINYINT
	AS
	BEGIN
		DECLARE @valor_retorno tinyint

		IF((SELECT COUNT(*) FROM Factura JOIN Item_Factura ON Factura.fact_numero = Item_Factura.item_numero WHERE (dbo.ES_COMPUESTO_POR_PRODUCTO(item_producto) = 1) AND (fact_numero = @factura)) > 0)
			SET @valor_retorno = 1
		ELSE
			SET @valor_retorno = 0
	...
	BEGIN
		...
		WHILE(@@FETCH_STATUS = 0)
			BEGIN
				...
				IF(dbo.TIENE_PRODUCTO_COMPONENTE(@factura_numero) = 0)
					BEGIN
				...
				ELSE
					BEGIN
						RAISERROR('La factura %s no pudo grabarse debido a que contiene un producto componente suelto', 16, 1, @factura_numero)
					END
